...
type widgetTypes = "number" | "slider" | "combo" | "text" | "toggle" | "button";
type SlotShape =
...
type WidgetCallback<T extends IWidget = IWidget> = (
  this: T,
  value: T["value"],
...
interface IButtonWidget extends IWidget<null, {}> {
  type: "button";
}
interface IToggleWidget extends IWidget<boolean, { on?: string; off?: string }> {
  type: "toggle";
}
interface ISliderWidget extends IWidget<number, { max: number; min: number }> {
  type: "slider";
}
interface INumberWidget extends IWidget<number, { precision: number }> {
  type: "number";
}
...
interface ITextWidget extends IWidget<string, {}> {
  type: "text";
}
...
type ContextMenuItem = IContextMenuItem | null;
...
declare const LiteGraph: {
  ...
  /* 自动加载槽类型，如果不使用请计算并静态设置相关变量 */
  auto_load_slot_types: boolean;
  /* 节点类的输入槽类型 */
  registered_slot_in_types: any;
  /* 节点类的输出槽类型 */
  registered_slot_out_types: any;
  /* 输入槽类型列表 */
  slot_types_in: any[];
  /* 输出槽类型列表 */
  slot_types_out: any[];
   ...
   */
  getNodeTypesInCategory(category: string, filter: string | undefined): LGraphNodeConstructor[];
   ...
   */
  getNodeTypesCategories(filter: string | undefined): string[];
...
declare class LGraph {
  ...
  protected _nodes: LGraphNode[];
  protected _groups: LGraphGroup[];
  protected _nodes_by_id: Record<number, LGraphNode>;
...
type SerializedLLink = [number, string, number, number, number, number];
declare class LLink {
  id: number;
  type: string;
  origin_id: number;
...
type SerializedLGraphNode<T extends LGraphNode = LGraphNode> = {
  id: T["id"];
  type: T["type"];
  pos: T["pos"];
  size: T["size"];
...
declare class LGraphNode {
   ...
   */
  addOutput(name: string, type: string | -1 | 0, extra_info?: Partial<INodeOutputSlot>): INodeOutputSlot;
  /**
   * add a new output slot to use in this node
   ...
   */
  addInput(name: string, type: string | number | -1, extra_info?: Partial<INodeInputSlot>): INodeInputSlot;
  /**
  ...
  onDrawTitle?(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void;
  ...
  onMouseDown?(
    event: MouseEvent & { canvasX: number; canvasY: number },
    pos: Vector2,
    graphCanvas: LGraphCanvas
  ): boolean | void;
  onMouseMove?(event: MouseEvent & { canvasX: number; canvasY: number }, pos: Vector2, graphCanvas: LGraphCanvas): void;
  onMouseUp?(event: MouseEvent & { canvasX: number; canvasY: number }, pos: Vector2, graphCanvas: LGraphCanvas): void;
  ...
  // -----------
  _waiting_actions: any[];
  doExecute(param: any, options?: any): void;
  actionDo(action: string, param: any, options: any, action_slot: number): void;
}
